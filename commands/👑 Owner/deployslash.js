    ...
    if (message.author.id != "950613304991182928")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setTitle("Only NotKisshore is allowed to deploy the SLASH-COMMANDS")
          .setDescription(`Go to the [Discord-Server](https://discord.gg/YtHDAEJWJ3), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:SH_Loading_Discord:955769949438238760> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:SH_Loading_Discord:955769949438238760> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
