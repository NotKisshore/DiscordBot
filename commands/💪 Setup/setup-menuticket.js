    ...
    try {
      ...
      async function first_layer() {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.getAuthor('Menu Ticket Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png', 'https://discord.gg/YtHDAEJWJ3'))
        ...
        collector.on('collect', menu => {
          if (menu?.user.id === cmduser.id) {
            ...
            content: `<:Wrong:955323096640921632> You are not allowed to do that! Only: <@${cmduser.id}>`,
            ephemeral: true
          });
        });
        ...
        collector.on('end', collected => {
          menumsg.edit({
            ...
            components: [],
            content: `<a:Tick:955323097915994164> **Selected: \`${collected && collected.first() && collected.first().values ? collected.first().values[0] : "Nothing"}\`**`
          }).catch(() => { });
        });
      }
      async function second_layer(SetupNumber) {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          //.setAuthor('Menu Ticket Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png', 'https://discord.gg/YtHDAEJWJ3')
          .setAuthor({ name: "Menu Ticket Setup", iconURL: "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png", url: "https://discord.gg/YtHDAEJWJ3" })
          .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))
        ...
        switch (optionhandletype) {
          case "Ticket Claim System": {
            ...
            */
            ...
            async function third_layer(SetupNumber) {
              ...
              let MenuEmbed = new Discord.MessageEmbed()
                .setColor(es.color)
                .setAuthor(SetupNumber + " Ticket Setup", "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/twitter/282/incoming-envelope_1f4e8.png", "https://discord.gg/YtHDAEJWJ3")
              ...
              collector.on('end', collected => {
                menumsg.edit({
                  embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                  components: [],
                  content: `${collected && collected.first() && collected.first().values ? `<a:Tick:955323097915994164> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**"}`
                })
              });
            }
          ...
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<:Wrong:955323096640921632> **You need to add at least 1 Open-Ticket-Option**")
            }
            ...
            if (collected && collected.first().content) {
              ...
              if (collected2 && (collected2.first().mentions.channels.size > 0 || message.guild.channels.cache.get(collected2.first().content?.trim()))) {
                ...
                return message.reply("<:Wrong:955323096640921632> **You did not ping a valid Channel!**")
              }
            } else {
              return message.reply("<:Wrong:955323096640921632> **You did not enter a Valid Message in Time! CANCELLED!**")
            }
          }
            break;
          case "Add Ticket Option": {
            let data = theDB.get(message.guild.id, pre + ".data");
            if (data.length >= 25) {
              return message.reply("<:Wrong:955323096640921632> **You reached the limit of 25 different Options!** Remove another Option first!")
            }
            ...
            if (collected && collected.first().content) {
              if (!collected.first().content.includes("++")) return message.reply("<:Wrong:955323096640921632> **Invalid Usage! Please mind the Usage and check the Example**")
              let value = collected.first().content.split("++")[0].trim().substring(0, 25);
              let index = data.findIndex(v => v.value == value);
              if (index >= 0) {
                return message.reply("<:Wrong:955323096640921632> **Options can't have the SAME VALUE!** There is already an Option with that Value!");
              }
          ...
          case "Edit Ticket Option": {
            ...
            let data = theDB.get(message.guild.id, pre + ".data");
            if (!data || data.length < 1) {
              return message.reply("<:Wrong:955323096640921632> **There are no Open-Ticket-Options to remove**")
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                ...
                async function handle_the_picks3(optionhandletype) {
                  ...
                  switch (optionhandletype) {
                    ...
                    case `Change Open Category`: {
                      ...
                      return message.reply("<:Wrong:955323096640921632> **Invalid Category-ID added**")
                    } break;
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `<a:Tick:955323097915994164> **Selected: \`${collected.size > 0 ? collected.first().values[0] : "NOTHING"}\`**`
              })
            });
          }
            break;
          case "Remove Ticket Option": {
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `<a:Tick:955323097915994164> **Selected: \`${collected.first().values[0]}\`**`
              })
            });
          }
            break;
