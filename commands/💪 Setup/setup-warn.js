    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor('Warn Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/prohibited_1f6ab?.png', 'https://discord.gg/YtHDAEJWJ3')
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content: `<:Wrong:955323096640921632> You already selected something, this Selection is now disabled!`, ephemeral: true})
              menuselection(menu);
            }
